...
namespace RootMotion.Demos {
	...
	/// <summary>
	/// AI character controller. This class is based on the ThirdPersonCharacter.cs of the Unity Exmaple Assets.
	/// </summary>
	public class AICharacter : CharacterBase {
		...
		public AIControl aiControl; // ai control
		...
		void FixedUpdate() {
			...
			*/
			...
			// Friction
			if (aiControl.state.move == Vector3.zero && groundDistance < airborneThreshold * 0.5f)
			{
				HighFriction();
			}
			else
			...
			bool stopSlide = onGround && aiControl.state.move == Vector3.zero && r.velocity.magnitude < 0.5f && groundDistance < airborneThreshold * 0.5f;
			// Individual gravity
			...
			if (onGround) {
				...
				if (!aiControl.state.jump) jumpReleased = true;

				//r.AddForce(gravity * gravityMultiplier);
				if (jumpReleased && aiControl.state.jump && !doubleJumped && doubleJumpEnabled) {
					jumpEndTime = Time.time + 0.1f;
					animState.doubleJump = true;

					Vector3 jumpVelocity = aiControl.state.move * airSpeed;
			...
			ScaleCapsule(aiControl.state.crouch? crouchCapsuleScaleMlp: 1f);
        ...
        protected virtual void Update() {
			...
			animState.crouch = aiControl.state.crouch;
		...
		private void MoveFixed(Vector3 deltaPosition) {
			...
			if (onGround) {
				...
				Vector3 groundMove = new Vector3 (aiControl.state.move.x * groundSpeed, 0f, aiControl.state.move.z * groundSpeed);
				...
				Vector3 airMove = V3Tools.ExtractHorizontal(aiControl.state.move * airSpeed, gravity, 1f);
		...
		private bool CanWallRun() {
			...
			if (aiControl.state.move.magnitude < wallRunMinMoveMag) return false;
			return true;
		}
		...
		private Vector3 GetMoveDirection() {
			...
			case MoveMode.Directional:
				moveDirection = Vector3.SmoothDamp(moveDirection, new Vector3(0f, 0f, aiControl.state.move.magnitude), ref moveDirectionVelocity, smoothAccelerationTime);
				moveDirection = Vector3.MoveTowards(moveDirection, new Vector3(0f, 0f, aiControl.state.move.magnitude), Time.deltaTime * linearAccelerationSpeed);
				return moveDirection * forwardMlp;
			case MoveMode.Strafe:
				moveDirection = Vector3.SmoothDamp(moveDirection, aiControl.state.move, ref moveDirectionVelocity, smoothAccelerationTime);
				moveDirection = Vector3.MoveTowards(moveDirection, aiControl.state.move, Time.deltaTime * linearAccelerationSpeed);
		...
		protected virtual void Rotate() {
			...
			if (aiControl.state.move == Vector3.zero) angle *= (1.01f - (Mathf.Abs(angle) / 180f)) * stationaryTurnSpeedMlp;
		...
		private Vector3 GetForwardDirection() {
			bool isMoving = aiControl.state.move != Vector3.zero;

			switch (moveMode) {
			case MoveMode.Directional:
				if (isMoving) return aiControl.state.move;
				return lookInCameraDirection? aiControl.state.lookPos - r.position: transform.forward;
			case MoveMode.Strafe:
				if (isMoving) return aiControl.state.lookPos - r.position;
				return lookInCameraDirection? aiControl.state.lookPos - r.position: transform.forward;
			}
		...
		protected virtual bool Jump() {
			...
			if (!aiControl.state.jump) return false;
			if (aiControl.state.crouch) return false;
