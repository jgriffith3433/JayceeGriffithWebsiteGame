var Lib_BEST_HTTP_WebGL_WS_Bridge =
{
	...
	{
		var urlStr = /*encodeURI*/(UTF8ToString(url))
					.replace(/\+/g, '%2B')
...
socket.socketImpl = new signalR.HubConnectionBuilder()
    .withUrl(urlStr)
    .withHubProtocol(new signalR.protocols.msgpack.MessagePackHubProtocol())
    .build();

socket.socketImpl.onclose(() => {
    console.log(id + ' WS_Create - onClose ' + e.code + ' ' + e.reason);
    //if (e.code != 1000)
    //{
    //	if (e.reason != null && e.reason.length > 0)
...
for (var i = 0; i < targets.Length; i++) {
    socket.socketImpl.on(targets[i], (data) => {
        console.log(targets[i] + ': ' + data);
...
WS_GetState: function (id) {
    ...
    if (typeof socket === 'undefined' || socket == null || socket.socketImpl == null || socket.socketImpl.state == signalR.HubConnectionState.Disconnected) {
        return 3; // closed
    }

    if (socket.socketImpl.state == "Connecting") {
        return 0;
    } else if (socket.socketImpl.state == "Connected") {
        return 1;
    } else if (socket.socketImpl.state == "Disconnecting") {
        return 2;
    } else if (socket.socketImpl.state == "Reconnecting") {
        return 0;
    } else {
        return 3; // closed
    }
...
WS_Send_String: function (id, str) {
    ...
    try {
        socket.socketImpl.invoke("SendPacket", str).catch(function (e) {
            ws._callOnError(socket.onError, id, ' ' + e.name + ': ' + e.message);
...
WS_Send_Binary: function (id, ptr, pos, length) {
    ...
    try {
        ...
        //socket.socketImpl.invoke("SendPacket", HEAPU8.buffer.slice(ptr + pos, ptr + pos + length)).catch (function (e) {
		socket.socketImpl.invoke("SendPacket", buff).catch(function (e) {
            ws._callOnError(socket.onError, id, ' ' + e.name + ': ' + e.message);
            return console.error(e.toString());
        });
    }
