...
public class MenuTutorial : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        m_ChooseGameArrow.SetActive(
            (TNManager.isConnectedToGameServer && m_ExampleMenu.MenuVisible && !inChannelOtherThanChat && string.IsNullOrEmpty(m_ExampleMenu.GetSelectedServerOrChannelName())) ||
            (!TNManager.isConnectedToGameServer && m_ExampleMenu.MenuVisible && string.IsNullOrEmpty(m_ExampleMenu.GetSelectedServerOrChannelName()) && m_ExampleMenu.ServerList != null && m_ExampleMenu.ServerList.list.size > 0)
        );
