...
public class ExampleCreate : MonoBehaviour
{
	...
	void OnClick ()
	{
		...
		if (TNManager.isConnectedToGameServer && !TNManager.IsInChannel(channelID)) return;
		...
		// Note that passing "channelID" is optional. If you don't pass anything, GNet will pick one for you.
		TNManager.Instantiate(channelID, "CreateColoredObject", prefabName, true, pos, rot, color, autoDestroyDelay);
	}
	...
	[RCC]
	static GameObject CreateColoredObject (GameObject prefab, Vector3 pos, Quaternion rot, Color c, float autoDestroyDelay)
	{
